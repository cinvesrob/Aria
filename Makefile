...
ALL_TARGETS:=lib/libAria.so utils examples tests docs params lib/libArNetworking.so swig arnetworking_docs arnetworking_wrappers clib/libArNetworking.a $(STATIC_TARGETS)
...
dist-all: lib/libAria.so params docs CommandLineOptions.txt.in \
  ...
  examples/demoStatic ArNetworking/examples/serverDemo \
  ArNetworking/examples/serverDemoStatic ArNetworking/examples/clientDemo \
  ArNetworking/examples/clientDemoStatic java python  arnetworking_wrappers \
  arnetworking_docs
endif
...
python/_AriaPy.so: obj/AriaPy_wrap.o lib/libAria.so 
	...
	$(CXX) -shared -o $(@) obj/AriaPy_wrap.o -Llib -lAria -lpthread -ldl -lrt
...
obj/AriaPy_wrap.o: python/AriaPy_wrap.cpp
	...
	$(CXX) -c $(BARECXXFLAGS) $(CXXINC) -DARIA_WRAPPER $(PYTHON_INCLUDE_FLAGS) $< -o $@
...
lib/libAriaJava.so: obj/AriaJava_wrap.o lib/libAria.so 
	$(CXX) -fPIC -shared -o $(@) obj/AriaJava_wrap.o -lpthread -ldl -lrt -Llib -lAria
...
obj/AriaJava_wrap.o: java/AriaJava_wrap.cpp
	...
	@ test -d "$(JAVA_INCLUDE)" 
	mkdir -p obj
	$(CXX) -c $(BARECXXFLAGS) $(CXXINC) -DARIA_WRAPPER -I$(JAVA_INCLUDE) -I$(JAVA_INCLUDE)/linux $< -o $@
...
install: all arnetworking_all
	...
	$(INSTALL) -D --strip -m 666 lib/libAria.so lib/libArNetworking.so $(DESTDIR)$(INSTALL_DIR)/lib/
	if test -f lib/libAriaJava.so; then $(INSTALL) --strip -m 666 lib/libAriaJava.so $(DESTDIR)$(INSTALL_DIR)/lib/; else echo Warning: lib/libAriaJava.so not found. Use \"make java\" to build.; fi
	if test -f lib/libArNetworkingJava.so; then $(INSTALL) --strip -m 666 lib/libArNetworkingJava.so $(DESTDIR)$(INSTALL_DIR)/lib/; else echo Warning: lib/libArNetworkingJava.so not found. Enter the ArNetworking directory and run \"make java\" to build.; fi
...
src-install:
	$(INSTALL) -m 775 -d $(DESTDIR)$(INSTALL_DIR)
	find    include src tests utils params docs examples advanced maps \
	        ArNetworking java javaExamples python pythonExamples  \
